      ...
      <v-col cols="12" md="9">
        <v-window v-model="tab" touchless vertical>
          ...
          >
            ...
            <br />
            <v-row class="pa-2">
              <template v-for="(a, i) in coursesThought">
                <v-col
                  :key="`coursesThought${i}-${a.id}`"
                  cols="11"
                  sm="6"
                  md="4"
                >
...
export default {
  ...
  async asyncData({ $strapi, params }) {
    try {
      ...
      let attendance = await $strapi.find("staff-attendances", {
        "user.id": params.id
      });

      let coursesThought = await $strapi.find("course-lecturers", {
        "teacher.id": params.id
      });
      return { user: u, coursesThought, courses, attendance };
    } catch (error) {
      console.log(error);
    }
  },
