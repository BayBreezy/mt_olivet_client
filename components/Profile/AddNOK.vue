<template>
  <div>
    ...
    >
      <v-card class="pb-4">
        <v-card-title>Add Next of Kin</v-card-title>
        <v-btn
          @click="dialog = false"
          icon
          absolute
  ...
  methods: {
    ...
    async CreateNOK() {
      ...
      try {
        ...
        let d = await this.$strapi.create("kins", {
          name: this.name,
        ...
        this.$store.commit("utils/Set_Snack", {
          show: true,
          color: "green",
          text: "Next of Kin Created."
        });
        this.dialog = false;
        this.$emit("NOKAdded", false);
      } catch (error) {
        this.$store.commit("utils/Set_Overlay", false);
        console.log(error);
